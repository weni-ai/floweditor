...
import { createSendMsgAction, createSendWhatsAppMsgAction } from 'testUtils/assetCreators';
import { setEmpty, set } from 'utils';
...
describe(SendWhatsAppMsgComp.name, () => {
  describe('render', () => {
    ...
    it('should render quick replies list when passed', () => {
      const { wrapper } = setup(false, { quick_replies: set(['Quick Reply 1']) });
