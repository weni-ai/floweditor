...
describe('helpers', () => {
  ...
  describe('initializeFlow', () => {
    ...
    it('should guess node types', () => {
      ...
      // guess a subflow
      expect(
        guessNodeType(createSubflowNode(createStartFlowAction()).node),
      ).toBe(Types.split_by_subflow);

      // guess a resthook
      expect(
        guessNodeType(createResthookNode(createCallResthookAction()).node),
      ).toBe(Types.split_by_resthook);

      // guess an airtime node
      expect(
        guessNodeType(
          createAirtimeTransferNode(createTransferAirtimeAction()).node,
        ),
      ...
      // guess random router
      expect(guessNodeType(createRandomNode(3).node)).toBe(
  ...
  describe('RenderNodeMap', () => {
    ...
    it('should identify collisions', () => {
      ...
      collides({ left: 0, top: 100, right: 200, bottom: 300 }, [
        'node0',
        'node1',
      ]);
      collides({ left: 0, top: 100, right: 200, bottom: 500 }, [
        'node0',
        'node1',
        'node2',
      ]);
    ...
    describe('getLocalizations', () => {
      ...
      it('should get localized cases', () => {
        ...
        expect((localizations[0].getObject() as Case).arguments).toEqual([
          'espanol case',
        ]);
